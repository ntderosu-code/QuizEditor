...
final class TextEntryAnswerKeyTests: XCTestCase {
    ...
    private func itemXML(_ type: QuizQuestionType, _ engine: CanvasQuizEngine) throws -> String {
        let package = try CanvasQTIExporter(engine: engine).makePackage(for: quiz(type))
    ...
    func testClassicGradesAgainstTheAnswerText() throws {
        for type in [QuizQuestionType.fillInBlank, .shortAnswer] {
            let xml = try itemXML(type, .classicQuizzes)
            XCTAssertTrue(
    ...
    func testClassicAcceptsEveryAnswerRow() throws {
        let xml = try itemXML(.shortAnswer, .classicQuizzes)
    ...
    func testNewQuizzesDeclaresAStringResponse() throws {
        for type in [QuizQuestionType.fillInBlank, .shortAnswer] {
            let xml = try itemXML(type, .newQuizzes)
    ...
    func testNewQuizzesMapsEveryAcceptedSpelling() throws {
        let xml = try itemXML(.fillInBlank, .newQuizzes)
    ...
    func testMisconceptionTagsStillDoNotExport() throws {
        ...
        let package = try CanvasQTIExporter(engine: .classicQuizzes).makePackage(for: q)
